-- ============================================================
-- AMTEC Calendar-Dispatch: Machine & Instrument Seed Data
-- Run AFTER 002_staff_and_enhancements.sql
-- ============================================================

-- ─── 1. Seed instruments into the instruments table ──────────────
-- These will appear in the instrument search dropdown on the dispatch form.
-- Skips any that already exist (matched by instrument_name).
...
FROM unnest(ARRAY[
  ...
  'Tachometer',
  'Thermometer',
  'Thermometer and/or Thermocouple wires',
  'Weighing Scale'
]) AS name
WHERE NOT EXISTS (
  SELECT 1 FROM instruments i WHERE i.instrument_name = name
);

-- ─── 2. Seed machine → instrument mappings ───────────────────────
